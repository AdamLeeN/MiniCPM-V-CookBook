...
def start_cpp_server(model_dir: str, gpu_devices: str, port: int):
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        cuda_lib_paths = [
            "/usr/lib/wsl/lib",  # 🔧 WSL GPU 直通真驱动（必须在最前面）
            cuda_env_path,
            bin_dir,  # libggml-cuda.so, libomni.so 等
            "/usr/lib/x86_64-linux-gnu",
            env.get('LD_LIBRARY_PATH', '')
        ]
        env["LD_LIBRARY_PATH"] = ":".join(p for p in cuda_lib_paths if p)
        # 🔧 [WSL/DrvFS 修复] 强制 CUDA backend 加载
    ...
    try:
        ...
        print(f"[DEBUG] ldd {server_bin}:", flush=True)
        for line in ldd_result.stdout.splitlines():
            if "cuda" in line.lower() or "ggml" in line.lower() or "not found" in line.lower():
    ...
    try:
        ...
        cuda_lines = [l for l in test_result.stderr.splitlines() if "cuda" in l.lower() or "gpu" in l.lower() or "device" in l.lower()]
        if cuda_lines:
            print(f"[DEBUG] llama-server --help CUDA output:", flush=True)
            for line in cuda_lines[:5]:
                print(f"  {line}", flush=True)
    except Exception as e:
        print(f"[DEBUG] llama-server test failed: {e}", flush=True)
    
    # 🔧 [WSL/DrvFS 修复] 如果运行在 WSL 的 /mnt/ 挂载盘上，复制到 Linux ext4 文件系统
    # DrvFS 的 9p 协议不支持某些 mmap 特性，CUDA kernel 加载会失败
    if platform.system() == "Linux" and server_bin.startswith("/mnt/"):
        import shutil
        wsl_bin_dir = f"/tmp/llamacpp_cuda_bin_{os.getpid()}"
        print(f"[WSL/DrvFS] Detected llama-server on DrvFS ({server_bin}), copying to {wsl_bin_dir}...", flush=True)
        try:
            if os.path.exists(wsl_bin_dir):
                shutil.rmtree(wsl_bin_dir)
            shutil.copytree(bin_dir, wsl_bin_dir, symlinks=True)
            # 确保可执行权限
            for f in os.listdir(wsl_bin_dir):
                fpath = os.path.join(wsl_bin_dir, f)
                if os.path.isfile(fpath):
                    os.chmod(fpath, os.stat(fpath).st_mode | 0o111)
            server_bin = os.path.join(wsl_bin_dir, os.path.basename(server_bin))
            bin_dir = wsl_bin_dir
            print(f"[WSL/DrvFS] Copied to {wsl_bin_dir}, new server_bin={server_bin}", flush=True)
            # 更新 model_path 为绝对路径（因为 cwd 变了）
            if not os.path.isabs(model_path):
                model_path = os.path.join(llamacpp_root, model_path)
        except Exception as e:
            print(f"[WSL/DrvFS] Copy failed, falling back to DrvFS: {e}", flush=True)
    ...
    # 🔧 [WSL/DrvFS 修复] 从 build/bin 目录启动，确保动态库正确加载
