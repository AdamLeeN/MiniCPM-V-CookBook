...
async def test_break():
    ...
    async def receive_audio(track):
        nonlocal audio_chunks, audio_samples, last_audio_time
        audio_stream = rtc.AudioStream(track)
        async for frame_event in audio_stream:
            audio_chunks += 1
            audio_samples += frame_event.frame.samples_per_channel
    ...
    print(f"  音频帧数: {audio_chunks}, samples: {audio_samples}")
    ...
    print(f"\n===== 发送打断指令 =====")
    print(f"打断前音频帧数(samples): {audio_before_break}")
    ...
    print(f"\n打断后音频帧数(samples): {audio_after_break}")
    print(f"打断后增加samples: {audio_increase}")
    ...
    for k, v in events.items():
        status = "✅" if v else "❌"
        print(f"  {status} {k}: {v}")
    print(f"  总音频帧数: {audio_chunks}, 总samples: {audio_samples}")
